...
class Server:
    ...
    def agregar_bots(self):
        ...
        while len(self.jugadores) < 4:
            bot_nombre = f"Bot-{contador_bots}"
            colores_usados = [str(c) for _, c in self.jugadores]
            todos_los_colores = ["rojo", "amarillo", "azul", "verde"]
            colores_disponibles = [color for i, color in enumerate(todos_los_colores, 1)
                                if str(i) not in colores_usados]
            ...
            if colores_disponibles:
                bot_color = todos_los_colores.index(colores_disponibles[0]) + 1  # Elige el primer color disponible y obtiene su índice + 1
                print(f"Agregando bot {bot_nombre} con color {bot_color}")
                self.jugadores.append((bot_nombre, bot_color))
                self.parques.agregar_jugador(bot_nombre, bot_color)
                self.player_colors_and_positions[bot_nombre] = (bot_color, [-1, -1, -1, -1])
                
                # Crear hilo para manejar movimientos del bot
                threading.Thread(target=self.manejar_bot, args=(bot_nombre, bot_color)).start()
                contador_bots += 1
    ...
    def manejar_bot(self, bot_nombre, bot_color):
        ...
        while not self.parques.ganador:
            try:
                ...
                print(f"Error obteniendo posiciones de {bot_nombre}")
                #Solicitar las posiciones a uno de los clientes nada más, al primero
            ...
            if jugador_actual and jugador_actual.nombre == bot_nombre:
                if jugador_actual.en_carcel:
                    for i in range(3):
                        ...
                        if self.parques.dados.es_par:
                            for ficha in jugador_actual.fichas:
                                self.parques.tablero.salir_de_carcel(ficha)
                            jugador_actual.en_carcel = False
                            turn_message = f"{bot_nombre} lanza {valor_dados} y ha salido de la cárcel."
                            time.sleep(1)
                            self.broadcast(turn_message)
                            print("Bot ha salido de la cárcel")
                            color, posiciones = self.player_colors_and_positions[bot_nombre]
                            posiciones = [0, 0, 0, 0]
                            self.player_colors_and_positions[bot_nombre] = (color, posiciones)
                            initial_positions_message = "Posiciones iniciales: "
                            for nombre, (color, posiciones) in self.player_colors_and_positions.items():
                                initial_positions_message += f"{nombre}.{color}.{posiciones};"
                            time.sleep(1)
                            ...
                            turn_message = f"{bot_nombre} lanza {valor_dados} y no ha podido salir de la cárcel."
                            time.sleep(0.2)
                            self.broadcast(turn_message)
                    if not self.parques.dados.es_par:
                        turn_message = f"{bot_nombre} no ha podido salir de la cárcel."
                        time.sleep(1)
                        self.broadcast(turn_message)
                        self.parques.cambiar_turno()
                        continue
                    ...
                    if self.parques.verificar_condicion_un_dado(jugador_actual):
                        valor_dados = self.parques.dados.lanzar_uno()
                    else:
                        valor_dados = self.parques.lanzar_dados()
                    turn_message = f"{bot_nombre} lanza {valor_dados}."
                    time.sleep(1)
                    self.broadcast(turn_message)
                    ...
                    if self.parques.dados.es_par:
                        ...
                        for ficha in jugador_actual.fichas:
                            if ficha.casilla == self.parques.tablero.casillas[TipoDeCelda.CARCEL][ficha.color - 1]:
                                self.parques.tablero.salir_de_carcel(ficha)
                                salio_de_carcel = True
                                self.player_colors_and_positions[bot_nombre] = (ficha.color, self.parques.obtener_posiciones_fichas())
                        
                        if not salio_de_carcel:
                            jugador_actual.pares_consecutivos += 1
                    ...
                    if self.parques.ganador:
                        ...
                        time.sleep(1)  # Pausa más larga antes de anunciar ganador
                        winner_message = f"El ganador es {bot_nombre}!"
                        self.broadcast(winner_message)
                        break
                    else:
                        fichas_en_carcel = []
    ...
    def mover_fichas_bot(self, bot_nombre, valor_dados):
        ...
        fichas_disponibles = [
            i for i, pos in enumerate(self.player_colors_and_positions[bot_nombre][1])
            if pos != -1 and not re.search(r'CIELO\s*\d+', str(pos))
        ]
        if fichas_disponibles:
            # Elegir una ficha aleatoria de las disponibles
            ficha_seleccionada1 = random.choice(fichas_disponibles)
            ficha_seleccionada2 = random.choice(fichas_disponibles)
            print(f"{bot_nombre} selecciona la ficha {ficha_seleccionada1}")

            pos_fichas = self.parques.movimiento_fichas(valor_dados[0], valor_dados[1], ficha_seleccionada1, ficha_seleccionada2)
            if not pos_fichas:
                self.intentos_fallidos += 1
                if self.intentos_fallidos == self.intentos_maximos:
                    self.intentos_fallidos = 0
            ...
            self.player_colors_and_positions[bot_nombre] = (
                self.player_colors_and_positions[bot_nombre][0], 
                pos_fichas
            )
            print("Movimiento de fichas exitoso")
            turn_message = f"{bot_nombre} lanza {valor_dados} y mueve sus fichas."
            time.sleep(0.2)
            self.broadcast(turn_message)
            return pos_fichas
        else:
